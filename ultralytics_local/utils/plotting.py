...
from ultralytics_local.utils import IS_COLAB, IS_KAGGLE, LOGGER, TryExcept, ops, plt_settings, threaded
from ultralytics_local.utils.checks import check_font, check_version, is_ascii
...
class Annotator:
    ...
    Examples:
        >>> from ultralytics_local.utils.plotting import Annotator
...
def save_one_box(
    ...
    Examples:
        >>> from ultralytics_local.utils.plotting import save_one_box
        >>> xyxy = [50, 50, 150, 150]
...
def feature_visualization_merged(x, module_type, stage, merge_mode="mean", save_dir=Path("runs/detect/exp")):
    ...
    for m in {"Detect", "Segment", "Pose", "Classify", "OBB", "RTDETRDecoder"}:  # all model heads
        if m in module_type:
            return
    if isinstance(x, torch.Tensor):
        _, _channels, height, width = x.shape  # batch, channels, height, width
        if height > 1 and width > 1:
            # Save path
