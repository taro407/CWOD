...
__all__ = ["PSASCSA", "SCSA", "C2f_SCSA1", "C2f_SCSA2"]
...
class Bottleneck_1(nn.Module):
    ...
    def forward(self, x):
        ...
        return (
            x + self.cv2(self.cv1(x)) if self.add else self.cv2(self.att(self.cv1(x)))
        )  # 使用add进行特征提取的时候利用SCSA机制.
...
class Bottleneck_2(nn.Module):
    ...
    def forward(self, x):
        """'forward()' applies the YOLO FPN to input data."""
        return (
            x + self.cv2(self.att(self.cv1(x))) if self.add else self.cv2(self.cv1(x))
        )  # 进行特征提取的时候利用SCSA机制,特征融合时不用SCSA机制.
