...
def act_layer(act='silu', inplace=True):
    a = act.lower()
    if a == 'relu': return nn.ReLU(inplace)
    if a == 'relu6': return nn.ReLU6(inplace)
    if a == 'leakyrelu': return nn.LeakyReLU(0.1, inplace)
...
class AKDC(nn.Module):
    def __init__(self, c, K=3, r=16):
        ...
        self.sq = nn.Sequential(
            nn.Conv2d(c, c, K, 1, autopad(K), groups=c, bias=False),
            nn.BatchNorm2d(c), nn.SiLU(inplace=True)
        )
        self.h = nn.Sequential(
            nn.Conv2d(c, c, (1, M), 1, (0, M // 2), groups=c, bias=False),
            nn.BatchNorm2d(c), nn.SiLU(inplace=True)
        )
        self.v = nn.Sequential(
            nn.Conv2d(c, c, (M, 1), 1, (M // 2, 0), groups=c, bias=False),
            nn.BatchNorm2d(c), nn.SiLU(inplace=True)
        )
        hid = max(c // r, 8)
        self.w = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            nn.Conv2d(c, hid, 1, bias=True), nn.SiLU(inplace=True),
