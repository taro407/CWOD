...
from ultralytics_local.nn.Addmodules1 import *
from ultralytics_local.nn.Addmodules import *
from ultralytics_local.nn.Addmodules.DA_Net import DualAttentionBlock, C3k2_DAB
from ultralytics_local.nn.Addmodules.DTAB import C3k2_DTAB
from ultralytics_local.nn.Addmodules.SEAM import SEAM
from ultralytics_local.nn.Addmodules.iAFF import iAFF, AFF
from ultralytics_local.nn.Addmodules1.VanillaNet import VanillaBlock
from ultralytics_local.nn.autobackend import check_class_names
from ultralytics_local.nn.modules import (
    AIFI,
    C1,
    C2,
    C2PSA,
...
from ultralytics_local.utils import DEFAULT_CFG_DICT, LOGGER, YAML, colorstr, emojis
from ultralytics_local.utils.checks import check_requirements, check_suffix, check_yaml
from ultralytics_local.utils.loss import (
    E2EDetectLoss,
...
from ultralytics_local.utils.ops import make_divisible
from ultralytics_local.utils.patches import torch_load
from ultralytics_local.utils.plotting import feature_visualization
from ultralytics_local.utils.torch_utils import (
    fuse_conv_and_bn,
    fuse_deconv_and_bn,
    initialize_weights,
    intersect_dicts,
...
class BaseModel(torch.nn.Module):
    ...
    def _profile_one_layer(self, m, x, dt):
        ...
        try:
            ...
            thop = None  # conda support without 'ultralytics_local-thop' installed
...
class RTDETRDetectionModel(DetectionModel):
    ...
    def init_criterion(self):
        ...
        from ultralytics_local.models.utils.loss import RTDETRDetectionLoss
...
class WorldModel(DetectionModel):
    ...
    def get_text_pe(self, text, batch=80, cache_clip_model=True):
        ...
        """
        from ultralytics_local.nn.text_model import build_text_model
...
class YOLOEModel(DetectionModel):
    ...
    def loss(self, batch, preds=None):
        ...
        """
        if not hasattr(self, "criterion"):
            from ultralytics_local.utils.loss import TVPDetectLoss
...
class YOLOESegModel(YOLOEModel, SegmentationModel):
    ...
    def loss(self, batch, preds=None):
        ...
        """
        if not hasattr(self, "criterion"):
            from ultralytics_local.utils.loss import TVPSegmentLoss
...
def torch_safe_load(weight, safe_only=False):
    ...
    Examples:
        >>> from ultralytics_local.nn.tasks import torch_safe_load
        >>> ckpt, file = torch_safe_load("path/to/best.pt", safe_only=True)
    """
    from ultralytics_local.utils.downloads import attempt_download_asset
    ...
    try:
        with temporary_modules(
            modules={
                "ultralytics_local.yolo.utils": "ultralytics_local.utils",
                "ultralytics_local.yolo.v8": "ultralytics_local.models.yolo",
                "ultralytics_local.yolo.data": "ultralytics_local.data",
            },
            attributes={
                "ultralytics_local.nn.modules.block.Silence": "torch.nn.Identity",  # YOLOv9e
                "ultralytics_local.nn.tasks.YOLOv10DetectionModel": "ultralytics_local.nn.tasks.DetectionModel",  # YOLOv10
                "ultralytics_local.utils.loss.v10DetectLoss": "ultralytics_local.utils.loss.E2EDetectLoss",  # YOLOv10
            },
        ):
            if safe_only:
        ...
        if e.name == "models":
            raise TypeError(
                emojis(
                    ...
                    f"\nRecommend fixes are to train a new model using the latest 'ultralytics_local' package or to "
...
def parse_model(d, ch, verbose=True):
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            args = [length]
        elif m in {LAE,SCSA,SELayerV2,FocalModulation,MLLAttention}:
            c2 = ch[f]
            args = [c2, *args]
        elif m is DySample:
            args = [ch[f]]
            ...
            {Detect, WorldDetect, YOLOEDetect, Segment, YOLOESegment, Pose, OBB, ImagePoolingAttn, v10Detect,DynamicHead}
        ):
    ...
    if isinstance(model, torch.nn.Module):  # PyTorch model
        ...
        for m in model.modules():
            if isinstance(m, (Segment, YOLOESegment)):
                ...
                return "obb"
            elif isinstance(m, (Detect, WorldDetect, YOLOEDetect, v10Detect,DynamicHead)):
                return "detect"
