...
from ultralytics_local.utils.checks import check_file, check_font, is_ascii
from ultralytics_local.utils.downloads import download, safe_download, unzip_file
from ultralytics_local.utils.ops import segments2boxes
...
def visualize_image_annotations(image_path: str, txt_path: str, label_map: dict[int, str]):
    ...
    from ultralytics_local.utils.plotting import colors
...
def check_cls_dataset(dataset: str | Path, split: str = "") -> dict[str, Any]:
    ...
    if not train_set.is_dir():
        ...
        if image_files := list(data_dir.rglob("*.jpg")) + list(data_dir.rglob("*.png")):
            from ultralytics_local.data.split import split_classify_dataset
...
class HUBDatasetStats:
    ...
    Examples:
        >>> from ultralytics_local.data.utils import HUBDatasetStats
    ...
    def get_json(self, save: bool = False, verbose: bool = False) -> dict:
        ...
        for split in "train", "val", "test":
            ...
            if self.task == "classify":
                from torchvision.datasets import ImageFolder  # scope for faster 'import ultralytics_local'
                ...
                from ultralytics_local.data import YOLODataset
    ...
    def process_images(self) -> Path:
        ...
        from ultralytics_local.data import YOLODataset  # ClassificationDataset
...
def compress_one_image(f: str, f_new: str = None, max_dim: int = 1920, quality: int = 50):
    ...
    Examples:
        >>> from pathlib import Path
        >>> from ultralytics_local.data.utils import compress_one_image
