...
from ultralytics_local.data.utils import polygons2masks, polygons2masks_overlap
from ultralytics_local.utils import LOGGER, IterableSimpleNamespace, colorstr
from ultralytics_local.utils.checks import check_version
from ultralytics_local.utils.instance import Instances
from ultralytics_local.utils.metrics import bbox_ioa
from ultralytics_local.utils.ops import segment2box, xywh2xyxy, xyxyxyxy2xywhr
from ultralytics_local.utils.torch_utils import TORCHVISION_0_10, TORCHVISION_0_11, TORCHVISION_0_13
...
class Compose:
    ...
    def __init__(self, transforms):
        ...
        Examples:
            >>> from ultralytics_local.data.augment import Compose, RandomHSV, RandomFlip
...
class Mosaic(BaseMixTransform):
    ...
    Examples:
        >>> from ultralytics_local.data.augment import Mosaic
...
class MixUp(BaseMixTransform):
    ...
    Examples:
        >>> from ultralytics_local.data.augment import MixUp
        >>> dataset = YourDataset(...)  # Your image dataset
...
class CutMix(BaseMixTransform):
    ...
    Examples:
        >>> from ultralytics_local.data.augment import CutMix
        >>> dataset = YourDataset(...)  # Your image dataset
...
class RandomHSV:
    ...
    Examples:
        >>> import numpy as np
        >>> from ultralytics_local.data.augment import RandomHSV
    ...
    def __call__(self, labels: dict[str, Any]) -> dict[str, Any]:
        ...
        if self.hgain or self.sgain or self.vgain:
            ...
            # lut_hue = ((x * (r[0] + 1)) % 180).astype(dtype)   # original hue implementation from ultralytics_local<=8.3.78
...
class RandomFlip:
    ...
    def __call__(self, labels: dict[str, Any]) -> dict[str, Any]:
        ...
        Args:
            ...
            'instances' (ultralytics_local.utils.instance.Instances): An object containing bounding boxes and optionally
                keypoints.

        Returns:
            (dict[str, Any]): The same dictionary with the flipped image and updated instances:
            'img' (np.ndarray): The flipped image.
            'instances' (ultralytics_local.utils.instance.Instances): Updated instances matching the flipped image.
...
class CopyPaste(BaseMixTransform):
    ...
    Examples:
        >>> from ultralytics_local.data.augment import CopyPaste
        >>> dataset = YourDataset(...)  # Your image dataset
...
def v8_transforms(dataset, imgsz: int, hyp: IterableSimpleNamespace, stretch: bool = False):
    ...
    Examples:
        >>> from ultralytics_local.data.dataset import YOLODataset
        >>> from ultralytics_local.utils import IterableSimpleNamespace
...
def classify_transforms(
    ...
    """
    import torchvision.transforms as T  # scope for faster 'import ultralytics_local'
