...
from ultralytics_local.utils import LOGGER, DataExportMixin, SimpleClass, ops
from ultralytics_local.utils.plotting import Annotator, colors, save_one_box
...
class BaseTensor(SimpleClass):
    ...
    def cuda(self):
        ...
        Examples:
            >>> import torch
            >>> from ultralytics_local.engine.results import BaseTensor
            >>> data = torch.tensor([[1, 2, 3], [4, 5, 6]])
            >>> base_tensor = BaseTensor(data, orig_shape=(720, 1280))
            >>> gpu_tensor = base_tensor.cuda()
...
class Masks(BaseTensor):
    ...
    def __init__(self, masks: torch.Tensor | np.ndarray, orig_shape: tuple[int, int]) -> None:
        ...
        Examples:
            >>> import torch
            >>> from ultralytics_local.engine.results import Masks
...
class Keypoints(BaseTensor):
    ...
    Examples:
        >>> import torch
        >>> from ultralytics_local.engine.results import Keypoints
